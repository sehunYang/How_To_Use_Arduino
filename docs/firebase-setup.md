...
이 문서는 How to use Arduino의 실제 Firebase/GCP 프로젝트 구성에서 사람이 해야 하는 단계만 담고 있습니다. Phase 0.4/0.5에 해당하는 수작업을 순서대로 나열했습니다.

## 1. Google 계정 선택 (가장 먼저 할 선행 조건)

- [ ] 학교 Workspace 계정과 개인 계정 중 무엇을 쓸지 결정합니다.
  - 학교 Workspace(Google Workspace for Education): 도메인 관리자가 결제/Blaze 업그레이드를 차단할 수 있습니다
  - 개인 계정(gmail.com 등): 소유자가 직접 제어 가능

- [ ] 학교 계정을 선택했다면:
  - 도메인 관리자에게 Blaze 플랜(종량제) 업그레이드 권한이 있는지 확인합니다
  - 차단되어 있으면 이 접근 방식 전체가 불가능하며, 별도의 폴백 전략이 필요합니다 (현재 문서 범위 외)
...
- [ ] 프로젝트 생성 직후 Blaze(종량제) 플랜으로 업그레이드합니다
...
> Firebase 프로젝트를 만든다고 Firestore 데이터베이스가 자동으로 생기지는 않습니다. 아래를 하지 않으면 8단계에서 "Databases" 페이지에 아무것도 보이지 않습니다.

- [ ] Firebase 콘솔 → 왼쪽 메뉴 **Firestore Database** → **데이터베이스 만들기**
- [ ] 모드는 **네이티브 모드(Native mode)**를 선택합니다. Datastore 모드가 아닙니다. 이 프로젝트의 클라이언트 SDK와 보안 규칙은 네이티브 모드 전용입니다
- [ ] 위치를 선택합니다. 한 번 정하면 바꿀 수 없습니다. `asia-northeast3`(서울)를 권장하며, 2단계의 프로젝트 기본 위치와 맞추면 좋습니다
- [ ] 보안 규칙 시작 모드는 "프로덕션 모드(잠금)"로 시작해도 됩니다. 저장소의 `firestore.rules`를 곧 배포해서 덮어쓰기 때문입니다 (문서 맨 아래 "배포해야 할 보안 규칙" 절 참고)
...
- [ ] 주의: GCP 예산 알림은 지출을 차단하지 않습니다. 이것은 알림일 뿐이며 수 시간 지연될 수 있습니다.
...
- [ ] Firebase 콘솔의 "Authentication > Sign-in method"에서:
  - "이메일/비밀번호" 공급자를 활성화합니다
  - "이메일 링크 로그인" 옵션은 비활성화합니다 (필요 없음)

### 4.2 관리자 계정 생성

- [ ] "Users" 탭에서 "사용자 추가"를 클릭하여 관리자용 계정 1개를 생성합니다
  - 예: `admin@example.com` / 강력한 비밀번호
  - 이 계정은 `/admin` 페이지에서 레시피 저작에 사용됩니다

### 4.3 커스텀 클레임 발급 (로컬 Admin SDK 사용)

- [ ] 프로젝트 설정에서 서비스 계정 키를 다운로드합니다
  - "서비스 계정" 탭 → "새 키 생성" → JSON 형식
  - 주의: 이 키는 GitHub, CI, 번들된 코드 어디에도 저장하지 마세요
  - 다운로드한 JSON 파일은 로컬에서만 사용하고 즉시 삭제합니다
...
- [ ] CI 서비스 계정을 별도로 생성합니다:
  - Firebase 콘솔의 "Users" 탭에서 자동화 전용 계정 1개 추가
  - 예: `ci-automation@example.com` / 자동 생성한 복잡한 비밀번호
  - 이 계정은 오직 GitHub Actions의 자동화 검증 작업에만 사용됩니다
  - 위 스크립트에서 이 계정의 UID에 `ci: true` 클레임을 설정합니다

- [ ] 주의: Admin SDK 서비스 계정 키는 GitHub Actions Secret에 저장하지 마세요.
  - 그 키는 보안 규칙을 우회하며 모든 보안 방어를 무력화합니다
...
- [ ] 다음 도메인을 추가합니다:
  - 커스텀 도메인을 연결한 경우(예: `shy.ai.kr`)에는 실제 서빙되는 그 도메인을 추가합니다. 이것이 우선입니다
  - `<github-username-or-org>.github.io` (GitHub Pages 기본 도메인)도 추가해 두면 안전합니다. 커스텀 도메인은 이 주소로 리다이렉트되지만, 전환 중 잠깐이라도 접근될 가능성을 막아 줍니다
  - 커스텀 도메인을 쓰는 경우 `.github/workflows/deploy.yml`의 `VITE_BASE_PATH`를 `/`로 두고 `public/CNAME`에 도메인을 넣어야 합니다(둘 다 이 저장소에 이미 반영됨). 그렇지 않으면 자산 경로가 `/<repo>/assets/...`로 잘못 잡혀 흰 화면이 뜹니다

- [ ] 이 단계를 건너뛰면 조용히 실패합니다.
  - 도메인이 승인되지 않으면 `/admin` 페이지의 로그인이 (유용한 오류 메시지 없이) 작동하지 않습니다
  - 학생 페이지는 정상이지만 관리자 기능이 차단됩니다
...
- [ ] 로컬에서 `cors.json` 파일을 생성합니다:
  ...
  - GitHub Pages 기본 주소로도 직접 접근하게 하려면 `https://sehunYang.github.io`를 허용합니다.
...
- [ ] CORS가 설정되지 않으면 브라우저에서 Storage의 배선도 이미지에 접근할 수 없습니다.
...
- [ ] 텔레메트리(이벤트) 쓰기는 fire-and-forget이며 학생 상호작용을 막지 않습니다.
  ...
  - 이벤트/통계 수집이 실패하면 `catch` 블록에서 조용히 무시합니다
  - 이는 설계된 제약이며 선택할 수 있는 항목이 아닙니다.

---

## 8. Firestore 예약 백업 및 복원 훈련

> Firestore는 콘솔에서 클릭만으로 되는 예약 백업 기능을 정식 제공하므로, 아래는 그 기능만 사용합니다. Cloud Scheduler, Pub/Sub, Cloud Functions는 만들지 않습니다.

### 8.1 예약 백업 켜기 (콘솔 클릭만)

- [ ] [Google Cloud 콘솔의 Firestore "Databases" 페이지](https://console.cloud.google.com/firestore/databases)를 엽니다
- [ ] 프로젝트의 데이터베이스(보통 `(default)`)를 찾습니다. 그 행의 "Scheduled backups"(예약 백업) 열에서 **Edit settings**를 클릭합니다
- [ ] 빈도를 선택합니다:
  - 매일(Daily)을 권장합니다. 학기 중에는 콘텐츠가 날마다 바뀔 수 있습니다
  - 매주(Weekly)를 원하면 요일도 선택
- [ ] 보존 기간(retention period)을 설정합니다 (최대 14주)
  - 특별한 이유가 없으면 4주 정도면 충분합니다
- [ ] **저장(Save)**을 클릭하면 즉시 활성화됩니다

- [ ] 정확한 시각은 지정할 수 없습니다. "매일" 백업은 날마다 다른 시각에 실행되며, 이것이 정상입니다.
- [ ] 이 기능은 Blaze 플랜에서만 동작합니다. 2단계에서 업그레이드했으므로 추가 조치는 필요 없습니다. 백업 저장 용량에 비례한 소액 비용이 발생하므로 [가격 정책](https://firebase.google.com/pricing)을 참고합니다.

### 8.2 복원 훈련 (한 번은 반드시 실행)

- [ ] 복원해 본 적 없는 백업은 백업이 아닙니다.
  - 최소 1회는 복원해 보고 성공을 기록합니다.

- [ ] 8.1에서 설정한 뒤 최소 하루가 지나 첫 백업이 하나 이상 생성된 다음에 진행합니다
- [ ] 같은 "Databases" 페이지 → 예약 백업 화면으로 돌아갑니다
- [ ] "Backups" 표에서 아무 백업 항목이나 선택하고, 작업(Actions) 열의 점 3개(⋮) 메뉴를 클릭합니다
...
- [ ] 명령의 새 데이터베이스 이름 자리(placeholder)를 원하는 이름(예: `restore-test`)으로 바꾸고 실행합니다
  - 복원은 새로운 데이터베이스에 씁니다. 운영 중인 데이터베이스를 덮어쓰지 않으므로 시험해도 됩니다
...
- [ ] `storage.rules`: Cloud Storage 보안 규칙. 기본 버킷이 아니라 6단계에서 만든 `<project-id>-wiring` 버킷에 적용됩니다
- [ ] `firebase.json`: Firebase 프로젝트 설정. 이미 `storage` 항목이 named target(`wiring`)을 쓰도록 구성되어 있습니다

### 최초 1회: Storage target 등록

Storage 규칙을 기본 버킷이 아니라 이름 지정 버킷(`<project-id>-wiring`)에 배포하려면 최초 1회 target을 등록해야 합니다. 등록 결과는 `.firebaserc`에 저장되어 저장소에 커밋됩니다.
...
`.firebaserc`의 기본 프로젝트는 로컬 에뮬레이터 전용 가짜 프로젝트(`*-test`)로 되어 있으므로, 실제 배포에는 반드시 `--project`를 명시해야 합니다.

Storage에 named target(`wiring`)을 등록한 뒤에는 `--only storage:rules`가 아니라 `--only storage:<target이름>`을 써야 합니다. `storage:rules`로 쓰면 "Could not find rules for the following storage targets: rules"라는 오해하기 쉬운 오류가 납니다 (`rules`를 target 이름으로 잘못 해석해서 나는 오류입니다).
...
Phase 0 완료 조건: 위 "released" 두 줄이 나오면 로컬 파일 그대로가 배포된 것입니다(CLI가 파일 바이트를 그대로 업로드하므로 별도 diff 불필요). 눈으로 한 번 더 확인하고 싶으면 Firebase 콘솔의 Firestore/Storage 각 "규칙" 탭에서 방금 배포한 내용이 보이는지 확인합니다.
...
Phase 1 개발(데이터 모델, 검증기)로 진행할 수 있습니다.
