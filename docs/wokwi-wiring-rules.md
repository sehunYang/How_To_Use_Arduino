...
`src/wokwi/partGeometry.ts`가 `wokwi-elements`의 실제 `pinInfo`를 보유합니다.
...
실제 피치는 Uno 헤더 9.5px, MPU6050 헤더 9.6px입니다. **`minimumClearance`(10px)보다
좁습니다.** 그래서 같은 헤더의 이웃 핀으로 가는 두 도선은 피치가 강제하는 간격을 예외로
인정합니다 — 부품이 물리적으로 허용하지 않는 간격을 요구하면 그 헤더는 배선 자체가
불가능해집니다.

### 레이아웃의 용도 선언

```ts
purpose: 'recipe'   // 학생이 만들고 촬영하는 회로. 좌표를 실물에 고정. 기본값
purpose: 'fixture'  // CI에서만 도는 리그. 촬영하지 않으므로 좌표 고정 생략
```

`fixture`는 형상 데이터가 없는 파트(브레드보드, 커스텀 칩)로 구성된 검증 전용 리그에만
씁니다. 이 경우 배치는 기계 검증이 아니라 **사람이 눈으로 확인한 것**이며, 아무도 그것을
보고 배선하지 않기 때문에만 허용됩니다.
