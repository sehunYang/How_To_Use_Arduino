...
## 2026-09-11 레시피를 단계별 화면으로 나눈 결정

레시피를 손에 들고 실측하면서 나온 판단이다. 한 화면이 준비물·배선·코드·탐구
가이드·측정을 모두 들고 있어 무엇을 할 차례인지가 화면 어디에도 없었다. 108편
기준으로 한 화면에 배선 7~36단계, 스케치 50~182줄, 가이드 4,076자가 함께 있었다.

- 레시피 주소는 **라우팅 화면**이 된다. `/recipes/:id`는 한눈에 보기와 단계 목록만
  두고, 각 단계는 자기 주소를 갖는다(`/parts` `/wiring` `/code` `/design`
  `/measure`). 화면에는 한 가지 일만 남고, 뒤로 가기와 새로 고침이 자리를 잃지
  않는다.
- 순서를 강제하지 않는다. 아이디어만 보러 온 학생은 허브에서 바로 4단계로 간다.
  위쪽 이동 줄은 건너뛸 때, 아래쪽 이동 줄은 이어서 갈 때 쓴다.
- **탐구 가이드를 재기 전과 재고 나서로 가른다.** 여덟 절이 정확히 넷씩 갈린다.
  이론·변인 설계·탐구 순서·실행 계획은 4단계, 측정값 읽는 법·데이터 처리와
  그래프·결과 점검·더 나아가기는 5단계다. 한 덩어리로 두었을 때에는 학생이 한 번도
  재보기 전에 '데이터 처리와 그래프'를 읽었다. 번호도 `4-1~4-4`, `5-1~5-4`로 나뉜다.
- 측정 화면은 값을 꺼내는 방법이 맨 위다. 시리얼 모니터에 값이 쌓인 상태로 오는
  화면이기 때문이다.
- 문제 해결과 도움 카드는 **모든 단계 화면**에 접힌 채로 붙는다. 막히는 순간은 어느
  단계에서든 오는데, 그때 다른 화면으로 보내면 하던 일을 잃는다.
- 다른 단계를 가리키는 문장은 링크가 된다. 스크롤로 닿을 수 없는 자리를 "화면 위쪽"
  이라 부르던 12곳이 그렇다.

## 2026-09-10 레시피 화면 정보 구조 결정

세 페르소나(아무것도 모르고 따라만 하는 학생 · 코드와 배선을 바꿔 보고 싶은 학생 ·
아이디어를 얻으러 온 학생)로 111개 레시피를 점검한 결과를 반영한다.

- 화면에는 번호 체계를 **하나만** 둔다. 탐구 가이드 안의 절은 화면 절의 하위 번호(`4-1`)를
  받고 제목도 한 단계 낮춘다. 같은 크기의 `## 1.`이 "4. 탐구 가이드" 밑에 다시 나오면
  위에서 아래로만 읽는 학생은 앞으로 되돌아간 줄로 안다.
- '한눈에 보기'(질문·재는 것·바꾸는 것·확인할 관계)는 준비물보다 **먼저** 둔다. 무엇을 왜
  재는지 모른 채 부품부터 챙기면 표만 채우고 끝난다.
- 한 가지 사실은 한 자리에만 둔다. 안전 안내는 배선을 시작하기 전 한 곳에서만 보여 주고,
  "다음에 무엇을 해 볼까"는 가이드의 '더 나아가기' 한 곳에만 둔다. 본문에는 원본을
  남겨 두되 화면이 그릴 때 빼낸다.
- 조립이 측정보다 먼저 나온다. 몇 번 반복할지 먼저 정하라는 뜻은 순서가 아니라 탐구 순서
  절의 머리말이 지킨다.
- 화면이 말하는 수치는 스케치에서 읽는다. 표본 간격을 센서 종류로 짐작하지 않는다.
- 목록·검색 카드가 알리는 한 줄은 그 탐구가 답하려는 질문이다. 응용 안내는 레시피마다
  같을 수 있어 카드를 구별해 주지 못한다.
- 배선과 코드가 같은 핀을 가리킨다는 사실을 화면에 드러낸다(`// @pin` 선언). 따라 하기만
  하는 학생에게는 필요 없으므로 접어 둔다.

## 2026-08-01 반복 실험(회차) 결정

- 레시피가 "길이별로 3회 반복"처럼 반복 측정을 요구하므로, 시리얼 데이터를 회차 단위로 여러 번 더할 수 있어야 한다.
- 회차를 겹치는 기준은 가로축 값이 아니라 측정 순번이다. `millis()`가 회차마다 밀려 가로축 값이 정확히 같은 경우가 드물다. 대신 회차별 가로축 값이 전체 범위의 5%를 넘게 벌어지면 경고한다.
...
- 화면의 기본값은 붙여넣고 바로 그래프다. 구간 자르기·열 더하기·격자 표처럼 손이 더 가는 기능은 "고급"으로 바꿀 때만 나타난다. 대부분의 탐구는 붙여넣기만으로 끝나는데, 그 사람들에게까지 식 입력란을 먼저 보여 주면 화면이 어려워 보인다.
- 다만 자료가 그 기능을 부르고 있을 때는 기본 화면에서도 한 줄로 알려 주고 켜는 단추를 둔다. 숫자 열이 하나뿐이거나, 회차마다 가로축 값이 크게 다른 경우가 그렇다.
...
- 절 번호는 화면에 있는 절만 세어 붙인다. 번호를 고정해 두면 고급을 끈 화면에서 3번 다음에 5번이 오는 목차가 된다.
...
- 계산 열은 한 행씩이 아니라 열 전체를 놓고 계산한다. 속도(`diff`), 공급 에너지(`cumsum`), 이동평균(`avg`), 영점 빼기(`mean`), 처음 값으로 나누기(`first`)는 그 행 하나만 봐서는 만들 수 없다.
...
- 격자 표에서 색은 거들 뿐이고 값은 언제나 숫자로 적는다.
...
- 계산에 실패한 행은 오류를 띄우지 않고 빈 칸으로 둔다. 0 이하의 로그처럼 값이 없는 행이 섞여도 나머지 행으로 그래프가 그려져야 한다.
...
- 배지는 색상에만 의존하지 않고 각 항목의 텍스트를 항상 제공한다.
