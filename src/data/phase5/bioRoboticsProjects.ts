...
export const biologyProjectRecipes: Recipe[] = [
  project({
    ...
    science: '환경 센서는 생장을 직접 측정하지 않습니다. 조도는 광합성에 이용되는 광합성유효복사(PAR)와 같지 않고, 상대습도는 온도에 의존합니다. 관찰된 상관관계를 한 요인의 인과효과로 단정하지 말고 식물 종류, 물, 토양을 통제해야 합니다.',
...
export const roboticsProjectRecipes: Recipe[] = [
  project({
    ...
    connections: [
      ...
      { from: 'DRIVER.IN2', to: 'UNO.D4', color: 'orange', text: '왼쪽 모터 방향 입력 IN2를 D4에 연결하세요. 모터 한 개에는 방향 입력이 두 개 필요합니다.' },
    ...
    science: '초음파는 부드럽거나 비스듬한 표면에서 반사가 센서로 돌아오지 않을 수 있습니다. 단일 전방 센서만으로는 측면과 낭떠러지를 알 수 없고 MPU6050도 절대 위치를 제공하지 않습니다. 이 예제는 안전 기능이 아닌 제한된 교실 실험입니다.',
