...
export const e5Recipe = createProjectRecipe({
  ...
  procedure: '세 센서의 방향과 높이를 같게 고정하고 위치 번호를 기록하세요. 센서를 나란히 두어 센서끼리 비교한 값 보정 비율을 먼저 구한 뒤 측정 위치로 옮기고 여러 시점의 평균을 비교하세요. 세 곳보다 많은 격자를 재려면 한 센서는 기준 위치에 고정한 채 나머지 두 센서를 옮겨 가며 회차를 나눠 재고, 각 회차 값을 기준 센서 값으로 나눠 회차 사이에 조명이 달라진 영향을 보정하세요.',
  science: 'TSL2591은 주소가 0x29로 고정되어 같은 I2C 통신선에 직접 여러 개 연결할 수 없습니다. 채널 선택 장치는 한 번에 한 채널만 통신선에 연결합니다. 센서 간 감도 차이와 입사각 차이는 공간 차이처럼 보일 수 있습니다.',
