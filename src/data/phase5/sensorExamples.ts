...
PIR 센서는 사람 자체를 식별하지 않고, 렌즈 구역 사이에서 변하는 적외선 복사를 감지해 디지털 HIGH를 냅니다. 전원을 켠 뒤 센서가 안정화되는 시간이 필요합니다.
...
:::`,
  ...
  example({
    ...
    wiring: [
      ...
      wire('DS18B20.DATA', 'UNO.D4', 'green', 'DATA를 D4에 연결하고 DATA와 5V 사이에 4.7 kΩ 풀업 저항을 연결하세요.', 2),
    ],
    sketch: s5Sketch,
...
INA219는 작은 션트 저항 양단의 전압 차로 전류를 구하고 버스 전압도 측정합니다. 전력은 같은 순간의 전압과 전류를 곱해 계산합니다.

:::callout warn
부하는 VIN+와 VIN- 사이에 직렬로 연결해야 합니다. 허용 공통모드 전압과 션트 전류 정격을 넘기지 마세요.
:::

:::toggle 단위 확인
코드의 전압은 V, 전류는 mA이므로 둘을 곱하면 mW가 됩니다. 에너지를 구하려면 전력을 시간에 대해 누적해야 합니다.
:::`,
    applicationGuide: '저항이나 LED 부하를 바꾸며 전압 강하, 전류, 전력의 관계를 비교하세요.',
  ...
  example({
    ...
    sketch: s8Sketch,
    tunables: [{ anchor: 'samplingIntervalMs', name: '측정 간격', hint: '적분 시간보다 길게 유지하세요.' }],
    body: `## 넓은 범위의 조도 측정하기

TSL2591은 전체광 채널과 적외선 채널을 함께 읽어 사람 눈의 감도에 가까운 lux를 계산합니다. 이득과 적분 시간은 어두운 곳의 분해능과 밝은 곳의 포화 사이를 조절합니다.

:::toggle 포화와 보정
원시 채널이 포화되면 lux 계산은 유효하지 않습니다. 광원의 스펙트럼, 입사각, 센서 덮개에 따라 표준 조도계와 차이가 생길 수 있습니다.
:::`,
    applicationGuide: '같은 광원을 거리별로 측정하고 포화되지 않는 이득과 적분 시간을 찾으세요.',
    troubleshooting: guidance('TSL2591', '강한 빛에서는 이득이나 적분 시간을 낮추고 고정 주소 0x29가 응답하는지 확인하세요.'),
  }),
  example({
    id: 'S9',
...
:::`,
    ...
    troubleshooting: [
      ...
      { symptom: 'I2C 장치가 불규칙하게 사라짐', cause: '풀업이 없거나 너무 강한 중복 풀업, 긴 배선, 전원 접촉 불량이 원인일 수 있습니다.', fix: '공통 GND와 전압을 확인하고 배선을 짧게 하며 전체 풀업 저항을 점검하세요.' },
    ],
  }),
  example({
    id: 'S10',
    ...
    sketch: s10Sketch,
    tunables: [{ anchor: 'zeroLevel', name: '영점 ADC 값', hint: '자석이 멀리 있을 때 평균값으로 바꾸세요.' }],
    body: `## 홀 센서로 상대 자기장 읽기

선형 홀 센서는 자기장 방향에 따라 영점 전압보다 높거나 낮은 전압을 냅니다. 자석이 없을 때의 ADC 평균을 영점으로 삼아 부호로 극성 방향을, 절댓값으로 상대 세기를 표시합니다.
...
:::toggle 물리량으로 보정하기
ADC 차이를 테슬라로 바꾸려면 해당 모듈 데이터시트의 감도와 실제 공급전압, 기준 자석 또는 교정 장비가 필요합니다. 이 예제의 relative_strength는 자기장 단위가 아닙니다.
:::`,
