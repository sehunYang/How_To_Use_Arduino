...
:::toggle 알아둘 한계
가만히 있는 사람은 놓칠 수 있고 난방기, 햇빛, 뜨거운 공기의 움직임에도 반응할 수 있습니다. 보안용 단독 판정 장치로 쓰면 안 됩니다.
:::`,
...
:::toggle 값 해석
ADC 값은 회로에서 CDS가 위쪽과 아래쪽 중 어디에 있는지에 따라 밝을수록 커지거나 작아집니다. 보정하지 않은 값을 lux라고 부르면 안 됩니다.
:::`,
...
TSL2591 두 개는 고정 주소 0x29를 사용하므로 같은 I2C 버스에 직접 병렬 연결하면 동시에 응답합니다. TCA9548A는 한 번에 선택한 하위 채널만 상위 버스에 연결해 충돌을 막습니다.
...
:::`,
    ...
    troubleshooting: [
      ...
      { symptom: 'I2C 장치가 불규칙하게 사라짐', cause: '신호선을 기본 HIGH 상태로 유지하는 저항이 없거나 너무 많이 겹쳐 있고, 배선이 길거나 전원 접촉이 불량할 수 있습니다.', fix: '공통 GND와 전압을 확인하고 배선을 짧게 하며 풀업 전체 저항값을 점검하세요.' },
    ],
  }),
  example({
    id: 'S10',
...
:::toggle 물리량으로 보정하기
ADC 값의 차이를 테슬라로 바꾸려면 해당 모듈 데이터시트의 감도와 실제 공급전압, 기준 자석 또는 교정 장비가 필요합니다. 이 예제의 relative_strength는 자기장 단위가 아닙니다.
:::`,
