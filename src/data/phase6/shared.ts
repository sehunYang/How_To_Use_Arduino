...
:::`,
    // 41개 레시피가 모두 같은 한 문장을 들고 있었습니다. 이 값은 목록 카드와
    // 검색이 레시피를 구별하는 데 쓰이므로, 똑같은 문장은 41장의 카드를 구별할 수
    // 없는 벽으로 만듭니다. 레시피마다 다른 `graph`(무엇을 그려 무엇을 읽는가)를
    // 그대로 씁니다. 없는 말을 지어내지 않으면서 41개가 서로 달라집니다.
    applicationGuide: definition.graph,
    troubleshooting,
    status: 'draft',
