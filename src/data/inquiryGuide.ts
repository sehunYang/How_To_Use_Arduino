...
function numbered(steps: string[], startAt = 1): string {
...
function sensorBridge(recipe: Recipe, plan: ExperimentPlan): string {
  ...
  const reading = plan === 'condition-comparison'
    ? '조건을 한 번에 하나씩만 바꾸고 측정값이 어느 방향으로 얼마나 달라지는지 비교하면 위 관계를 확인할 수 있습니다.'
    : '먼저 시각이 붙은 원시 측정값을 그대로 저장하고, 사건 전후 또는 기록 구간 전체의 변화 모양을 나중에 분석해야 위 관계를 확인할 수 있습니다.'

  return `

### 센서는 무엇을 대신해 주나요

눈과 손으로는 빠른 변화나 작은 차이를 일정한 기준으로 비교하기 어렵습니다. 이 탐구에서 쓰는 센서는 ${names}입니다. 이 센서가 숫자로 바꿔 주는 값은 ${measured}이며, 사람이 세거나 재지 않아도 같은 간격으로 값과 발생 시각을 함께 남깁니다. ${reading}`
}
...
function theorySection(recipe: Recipe, plan: InquiryPlan, kind: ExperimentPlan): Section {
  return {
    title: '과학 이론 쉽게 이해하기',
    body: `이 탐구는 다음 사실을 확인하기 위한 것입니다.

> **${plan.relation}**

아래 용어와 식을 먼저 읽고 어떤 결과가 나올지 예상한 다음 측정을 시작하세요. 예상과 다른 결과가 나오면 그 이유를 찾는 것이 이 탐구의 가장 중요한 부분입니다.
...
function variableSection(plan: InquiryPlan): Section {
  const controls = plan.variables.controls.map((item, index) => `${index + 1}) ${item}`).join(' ')
  return {
    title: '변인 설계',
    body: `바꾸는 것을 하나로 좁혀야 결과의 원인을 말할 수 있습니다. 아래 표를 실험 노트에 그대로 옮겨 적고 시작하세요.

| 구분 | 이 탐구에서는 |
|:---|:---|
| 독립 변인 — 내가 단계적으로 바꾸는 것 | ${plan.variables.independent} |
| 종속 변인 — 센서가 재는 것 | ${plan.variables.dependent} |
| 통제 변인 — 끝까지 같게 유지할 것 | ${controls} |`,
  }
}
...
function procedureSection(authored: AuthoredBody): Section {
  ...
  return { title: '측정 순서', body: numbered(steps) }
}
...
function columnSection(recipe: Recipe): Section {
  ...
  return {
    title: '측정값 읽는 법',
    body: `시리얼 모니터의 첫 줄이 열 이름이고 그 아래가 측정값입니다. 그대로 복사해 표 계산 프로그램에 붙여 넣으면 됩니다.
...
function analysisSection(plan: InquiryPlan, authored: AuthoredBody): Section {
  ...
  return {
    title: '데이터 처리와 그래프',
    body: `저장한 CSV를 열어 아래 순서대로 계산하세요. 스케치에서 미리 계산하지 않고 원시값을 남겨 두었기 때문에, 중간에 방법을 바꿔도 다시 측정할 필요가 없습니다.
...
function checkpointSection(plan: InquiryPlan): Section {
  ...
  return {
    title: '결과 점검',
    body: `예상과 다른 결과도 훌륭한 자료입니다. 다만 원인을 찾아 적어 두어야 결론이 됩니다.
