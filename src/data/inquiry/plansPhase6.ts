...
export const phase6Plans: Record<string, InquiryPlan> = {
  ...
  's12-dual-mpu6050-address': {
    ...
    variables: {
      ...
      dependent: '두 주소의 가속도 값과 두 값이 가장 잘 겹치는 시간차',
      controls: ['두 센서의 고정 방법과 방향', '표본 간격', '공급 전압'],
    },
    analysis: [
      'I2C 스캔으로 0x68과 0x69가 모두 보이는지 먼저 확인합니다.',
      '한쪽 센서만 기울여 그 주소의 값만 변하는지 확인해 이름표가 맞는지 검증합니다.',
      '두 센서를 같은 물체에 붙여 동시에 흔들고, 두 곡선을 한 그래프에 겹쳐 그립니다.',
      '한 곡선을 시간 방향으로 조금씩 밀어 가장 잘 겹치는 지점을 찾고, 그 이동량을 두 센서의 측정 시간차로 기록합니다.',
    ],
    checkpoints: [
    ...
    extensions: {
      ...
      broaden: '표본 간격을 줄여 두 센서의 측정 시간차가 얼마나 작아지는지 확인하세요.',
  ...
  's13-mpu-aux-tsl2591': {
    ...
    measures: '보조 통신선에 붙은 조도센서의 값과 그 갱신 시각',
    changes: '보조 센서에 접근하는 방식(직접 연결 모드와 제어 모드)',
    relation: '통신선이 한 단계 더 거칠수록 값이 갱신되기까지 지연이 생긴다',
    concepts: ['i2cAddress', 'multiplexer', 'samplingInterval', 'illuminance'],
    variables: {
      independent: '접근 모드 2가지와 보조 통신선 갱신 주기 설정',
      dependent: '조도값이 실제 변화 뒤 몇 ms 만에 바뀌는지',
      controls: ['조도를 바꾸는 방법', 'TSL2591 전원 전압(3.3V)', '측정 간격'],
    },
    analysis: [
      'TSL2591 전원이 3.3V인지 먼저 확인합니다. 5V로 넣으면 보조 통신선에 과전압이 걸립니다.',
      '조명을 끄고 켜는 시각을 기록하며 light_raw가 바뀐 시각을 함께 봅니다.',
      '두 시각의 차이를 지연으로 정의하고 10회 반복해 평균과 최댓값을 구합니다.',
      '두 접근 모드에서 각각 같은 실험을 해 지연을 비교합니다.',
    ],
    checkpoints: [
    ...
    extensions: {
      ...
      broaden: '보조 센서를 주 통신선에 직접 붙였을 때의 지연과 비교해 계층 구조의 대가를 정리하세요.',
  ...
  's14-tca-address-reset': {
    ...
    checkpoints: [
      { sign: '리셋 후에도 채널이 열려 있습니다', meaning: '리셋 펄스가 너무 짧았을 수 있습니다. 펄스 길이를 늘려 다시 시도하세요.' },
      { sign: '두 모듈의 복구 시간이 크게 다릅니다', meaning: '배선 길이나 풀업 저항 조건이 다를 수 있습니다. 두 모듈의 배선을 같게 맞추고 다시 재세요.' },
    ],
    extensions: {
      immediate: '리셋 펄스 길이를 절반으로 줄여 복구 성공률이 떨어지는지 확인하세요.',
  ...
  'ph03-projectile-motion': {
    ...
    measures: '수평 이동 거리와 이륙·착지 순간의 가속도 사건',
    changes: '발사 속도(수평 방향)',
    ...
    variables: {
      independent: '발사 속도 3단계',
      dependent: '수평 이동 거리와 비행 시간',
      controls: ['발사 높이', '발사각', '물체의 질량과 모양', '포획 상자 위치'],
    },
    analysis: [
      '이륙과 착지 순간의 가속도 봉우리 사이 시간을 비행 시간으로 읽습니다.',
      '수평 이동 거리를 비행 시간으로 나눠 초기 수평 속도를 구합니다.',
      '$h=\\tfrac{1}{2}gt^2$에 비행 시간을 넣어 계산한 높이를 실제 발사 높이와 비교합니다.',
      '속도 조건별로 비행 시간이 거의 같은지 확인합니다. 이것이 수평과 수직이 서로 독립이라는 증거입니다.',
      '측정한 수평 거리와 예측한 거리의 차이를 표로 정리합니다.',
    ],
    checkpoints: [
      { sign: '속도를 올렸더니 비행 시간이 길어집니다', meaning: '발사각이 완전히 수평이 아니었을 수 있습니다. 각도를 다시 맞추세요.' },
      { sign: '가속도 봉우리를 찾을 수 없습니다', meaning: '표본 간격이 너무 깁니다. 간격을 줄이고 센서를 단단히 고정하세요.' },
    ],
    extensions: {
      immediate: '발사 높이만 2배로 올려 비행 시간이 약 1.4배가 되는지 확인하세요.',
  ...
  'ph04-momentum-collision': {
    ...
    variables: {
      ...
      dependent: '충돌 전후 각 카트의 속도와 운동량의 합',
      controls: ['충돌 직전 속도', '트랙의 수평 상태', '센서 부착 방향', '표본 간격'],
    },
    analysis: [
      '두 센서 주소가 0x68과 0x69로 나뉘어 각각 읽히는지 먼저 확인합니다.',
      '충돌 전 정지 구간의 평균을 각 축에서 빼 영점을 맞춥니다.',
      '충돌 직전과 직후의 짧은 구간에서만 가속도를 시간에 대해 쌓아 속도를 구합니다.',
      '질량을 곱해 각 카트의 운동량을 구하고, 충돌 전후 합계를 비교합니다.',
      '두 합계의 차이를 충돌 전 합계로 나눠 상대 차이를 구하고, 충돌 방식별로 표로 정리합니다.',
    ],
    checkpoints: [
      { sign: '충돌 후 운동량 합이 20% 이상 커집니다', meaning: '적분 구간이 너무 길어 영점 오차가 쌓인 것입니다. 구간을 짧게 다시 잡으세요.' },
  ...
  'ph07-centripetal-acceleration': {
    ...
    extensions: {
      ...
      broaden: '회전 속도를 더 높여 가속도가 센서의 측정 한계에 닿는 지점을 찾으세요.',
  ...
  'ph09-friction-coefficients': {
    ...
    formula: {
      expression: '$\\mu_s\\approx\\tan\\theta_c,\\quad \\mu_k=\\dfrac{g\\sin\\theta-a}{g\\cos\\theta}$',
      symbols: [
        { symbol: '$\\theta_c$', meaning: '미끄러지기 시작한 각도', unit: '°' },
        { symbol: '$\\mu_s$', meaning: '정지 마찰계수', unit: '-' },
        { symbol: '$\\mu_k$', meaning: '운동 마찰계수', unit: '-' },
        { symbol: '$a$', meaning: '미끄러지는 동안의 가속도', unit: 'm/s²' },
      ],
    ...
    analysis: [
      '판을 아주 천천히 올리며 센서 각도를 계속 기록하고, 움직이기 시작한 순간의 각도를 읽습니다.',
      '재료마다 5회 반복해 시작각의 평균과 퍼진 범위를 구합니다.',
      '시작각의 탄젠트로 정지 마찰계수를 구합니다.',
      '시작각보다 조금 큰 각도에서 미끄러뜨려 가속 구간의 평균 가속도를 구합니다.',
  ...
  'ph10-rotational-damping': {
    ...
    analysis: [
      '같은 처음 각속도에서 출발하도록 매번 같은 방법으로 회전시킵니다.',
    ...
    checkpoints: [
      ...
      { sign: '홀 펄스가 마지막에 끊깁니다', meaning: '속도가 너무 느려져 펄스 간격이 길어진 것입니다. 자이로 자료로 이어서 분석하세요.' },
    ],
    extensions: {
  ...
  'ph11-specific-heat': {
    ...
    analysis: [
      '가열을 시작한 직후, 온도가 거의 직선으로 오르는 구간을 고릅니다.',
  ...
  'ph16-altitude-pressure': {
    ...
    analysis: [
      ...
      '기울기의 크기를 $g$로 나눠 공기 밀도를 추정하고 1.2 kg/m³와 비교합니다.',
  ...
  'ph19-kirchhoff-laws': {
    ...
    analysis: [
      ...
      '전원 전압에서 각 부품의 전압 강하를 차례로 빼면 0이 되는지 확인합니다.',
  ...
  'ph20-joule-heating': {
    ...
    analysis: [
      ...
      '냉각 구간의 온도 차에 로그를 취해 냉각 상수를 구하고, 열손실 몫을 따로 평가합니다.',
    ],
    checkpoints: [
  ...
  'ph26-rotating-magnet-signal': {
    ...
    extensions: {
      immediate: '자석 하나의 극을 뒤집어 신호가 어떻게 달라지는지 관찰하세요.',
  ...
  'ph27-magnetic-shielding': {
    ...
    analysis: [
      ...
      '재료마다 같은 자리에 넣고 30개 표본의 평균에서 영점을 뺀 값을 구합니다.',
  ...
  'ph30-reflection-intensity-angle': {
    ...
    variables: {
      independent: '센서 각도(예상 반사각 ±40°를 10° 간격)와 시료 2~3가지',
  ...
  'ph32-aperture-light': {
    ...
    checkpoints: [
      ...
      { sign: '가장 큰 구멍에서 값이 더 이상 안 커집니다', meaning: '센서가 측정 범위를 넘었습니다. 광원을 멀리 두거나 증폭 설정을 낮추세요.' },
    ],
    extensions: {
  ...
  'ph33-light-source-stability': {
    ...
    extensions: {
      immediate: '표본 간격을 절반으로 줄여 새로 보이는 떨림이 있는지 확인하세요.',
  ...
  'ph34-torricelli-drain': {
    ...
    analysis: [
      '센서에서 수면까지의 거리를 용기 높이에서 빼 수면 높이로 바꿉니다.',
      '수면 높이를 시간에 대해 그려 곡선임을 확인합니다.',
      '$\\sqrt{h}$를 시간에 대해 다시 그리면 직선이 되어야 합니다.',
      '직선의 기울기로 배출구의 유효 넓이를 추정하고 실제 구멍 넓이와 비교합니다.',
  ...
  'ph35-temperature-speed-of-sound': {
    ...
    analysis: [
      ...
      '20 °C 고정 음속으로 계산한 거리와 실제 거리의 차이를 온도에 대해 그립니다.',
      '위 식으로 음속을 온도에 맞춰 계산한 뒤 거리를 다시 구합니다.',
