...
export const phase6Plans: Record<string, InquiryPlan> = {
  ...
  's11-tsl2591-interrupt': {
    ...
    setup: [
      '브레드보드에 TSL2591을 꽂고 수광창이 천장을 향하도록 세웁니다.',
      '신호선을 기본 HIGH 상태로 유지하는 10 킬로옴 저항을 INT 선과 3.3V 사이에 꽂습니다.',
      '책상 스탠드를 센서 위에 고정하고, 빛을 가릴 종이컵을 옆에 준비합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '컵으로 덮은 상태와 스탠드를 켠 상태의 값을 각각 읽어, 임계값을 그 사이로 정합니다.',
    ],
    variables: {
    ...
    analysis: [
      ...
      '기준을 넘은 첫 표본의 시각과 알림 시각의 차이를 구해 응답 지연으로 정리합니다.',
  ...
  's12-dual-mpu6050-address': {
    ...
    setup: [
      '브레드보드에 MPU6050 두 개를 서로 떨어뜨려 꽂고 1번, 2번 표시를 붙입니다.',
      '2번 센서의 AD0를 아두이노 3.3V 핀에 연결해 주소를 0x69로 바꿉니다.',
      'AD0를 5V에 연결하지 않았는지 다시 확인합니다. 5V를 넣으면 센서가 손상됩니다.',
      '두 센서를 같은 판 위에 나란히 붙여 같은 방향을 보게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '판을 수평으로 둔 채 두 센서의 값을 읽어, 두 줄이 모두 나오는지 확인합니다.',
    ],
    variables: {
  ...
  'p9-motion-interrupt': {
    ...
    setup: [
      '카트나 진자를 준비하고 부딪힐 벽 쪽에 완충재를 붙입니다.',
      'MPU6050을 카트에 테이프로 단단히 붙이고, 센서의 x축이 부딪히는 방향과 나란해지도록 돌립니다.',
      '아두이노와 브레드보드도 카트에 싣고 고정합니다.',
      'USB 케이블이 카트를 잡아당기지 않도록 위쪽에서 넉넉히 늘어뜨립니다.',
      '카트를 출발시킬 자리를 표시해 매번 같은 곳에서 놓을 수 있게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 115200 baud로 엽니다.',
      '카트를 정지시킨 채 2초 이상 값을 읽어 충돌 전 평균값을 적어 둡니다.',
    ],
    variables: {
    ...
    analysis: [
      '시리얼 모니터를 115200으로 맞추고, 충돌 구간에 표본이 20개 이상 들어가는지 확인합니다.',
  ...
  's13-mpu-aux-tsl2591': {
    ...
    setup: [
      '브레드보드에 MPU6050을 꽂고 그 옆에 TSL2591을 꽂습니다.',
      'TSL2591의 전원은 반드시 아두이노 3.3V 핀에서 가져옵니다. MPU6050의 보조 통신선이 3.3V로 동작하기 때문입니다.',
      '5V 핀에서 TSL2591로 가는 선이 없는지 다시 확인합니다.',
      '두 센서를 같은 판 위에 붙여 흔들리지 않게 고정합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '판을 수평으로 둔 채 값을 읽어 가속도와 조도 두 값이 모두 나오는지 확인합니다.',
    ],
    variables: {
    ...
    analysis: [
      'TSL2591 전원이 3.3V인지 확인합니다. 5V로 넣으면 보조 통신선에 과전압이 걸립니다.',
  ...
  's14-tca-address-reset': {
    ...
    setup: [
      '브레드보드에 TCA9548A 두 개를 서로 떨어뜨려 꽂고 1번, 2번 표시를 붙입니다.',
      '두 장치의 주소 설정 핀을 서로 다르게 연결해 주소가 겹치지 않게 만듭니다.',
      '각 장치의 주소를 노트에 적어 둡니다. 코드가 어느 주소를 부르는지 대조할 때 씁니다.',
      'RESET 선을 아두이노 디지털 핀에 연결해 코드에서 복구를 시킬 수 있게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '두 장치가 각각 다른 주소로 응답하는지 값을 보고 확인합니다.',
    ],
    variables: {
    ...
    checkpoints: [
      { sign: '리셋 후에도 채널이 열려 있습니다', meaning: '리셋 선이 LOW로 내려가지 않은 것입니다. RST 배선과 핀 번호를 확인하세요. 이 부품의 최소 펄스 폭은 나노초 수준이라 펄스 길이(10 µs)가 짧아서 실패하는 일은 없습니다.' },
  ...
  'p10-eight-point-light-field': {
    ...
    setup: [
      '측정할 공간에 여덟 지점을 격자로 정하고 바닥에 1번부터 8번까지 표시를 붙입니다.',
      'TSL2591 여덟 개에 같은 번호표를 붙이고 어느 센서가 몇 번 채널인지 표로 적습니다.',
      '여덟 센서를 모두 같은 높이의 받침에 올리고 수광창이 같은 방향을 보게 맞춥니다.',
      '대형 브레드보드에 TCA9548A를 꽂고 채널 자리를 번호 순서대로 정리합니다.',
      '화면 위쪽 **2. 배선하기**를 끝까지 마친 뒤, 각 센서의 SDA와 SCL이 같은 번호의 SD와 SC 자리에 들어갔는지 여덟 쌍 모두 확인합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '여덟 센서를 한자리에 모아 같은 빛을 받게 하고 값을 읽어, 센서마다의 차이를 보정 비율로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph01-uniform-motion': {
    ...
    setup: [
      '수평한 책상 위에 1 m 이상의 직선 트랙을 놓고, 트랙이 한쪽으로 기울지 않았는지 카트를 가만히 올려 확인합니다.',
      '트랙의 한쪽 끝에 HC-SR04를 트랙을 따라 정면을 보도록 세워 고정합니다.',
      '카트 뒤쪽에 가로세로 10 cm 이상인 평평한 카드를 세워 붙여 초음파 반사판으로 만듭니다.',
      '카드 면이 센서 정면과 나란하도록 각도를 맞춥니다.',
      '트랙에 출발선과 도착선을 표시하고 두 선 사이 거리를 자로 재어 적습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '카트를 출발선에 세워 둔 채 거리값을 읽어, 실제로 잰 거리와 맞는지 확인합니다.',
    ],
    variables: {
    ...
    analysis: [
      ...
      '직선이 자료를 얼마나 잘 설명하는지($R^2$)를 적습니다.',
  ...
  'ph02-newton-second-law': {
    ...
    setup: [
      '수평한 책상 위에 직선 트랙을 놓고 한쪽 끝에 도르래를 고정합니다.',
      'MPU6050을 카트 위에 붙이고, 센서의 x축이 카트가 굴러가는 방향과 나란해지도록 돌립니다.',
      '카트에 실을 묶고 실을 도르래에 걸어 반대쪽에 추를 답니다.',
      '카트 자체의 질량을 저울로 재어 적고, 얹을 질량추도 각각 재어 적습니다.',
      'USB 케이블이 카트를 잡아당기지 않도록 위쪽에서 넉넉히 늘어뜨립니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '카트를 정지시킨 채 값을 읽어 x축 가속도가 0 근처인지 확인합니다. 0이 아니면 트랙이 기울어 있는 것이므로 받침을 조절합니다.',
    ],
    variables: {
    ...
    analysis: [
      ...
      '트랙이 수평인지 확인하고, 기울어졌다면 그 성분도 빼야 합니다.',
  ...
  'ph03-projectile-motion': {
    ...
    setup: [
      '수평한 책상 끝에 낮은 경사로를 놓아 공이 굴러 내려와 책상 위를 지나가게 만듭니다.',
      'MPU6050을 책상 위에 올려놓고 두 축 값을 읽어 책상이 수평인지 확인합니다.',
      '책상 바닥에서 책상 윗면까지의 높이를 줄자로 재어 적습니다.',
      'HC-SR04를 책상 위에서 굴러오는 공을 향하도록 고정합니다. 날아가는 공을 쫓지는 않습니다.',
      '책상 아래 바닥에 흰 종이를 깔고 그 위에 먹지를 뒤집어 올려 착지점이 찍히게 합니다.',
      '착지 구역 둘레에 포획 상자를 놓고, 사람이 지나다니지 않는 방향으로 발사 방향을 잡습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ],
    variables: {
  ...
  'ph04-momentum-collision': {
    ...
    setup: [
      '수평한 책상 위에 직선 트랙을 놓고 카트 두 대를 올립니다.',
      '카트마다 MPU6050을 하나씩 붙이고, 각각 1번 카트와 2번 카트로 표시합니다.',
      '2번 카트의 센서는 AD0를 3.3V에 연결해 주소를 다르게 만듭니다. 5V에 연결하면 센서가 손상됩니다.',
      '두 센서의 x축이 모두 트랙 방향과 나란해지도록 돌립니다.',
      '두 카트가 부딪히는 면에 용수철이나 스펀지 범퍼를 붙여 충돌이 천천히 일어나게 합니다.',
      '카트 두 대의 질량을 각각 저울로 재어 적습니다.',
      '리드선이 카트를 잡아당기지 않도록 위쪽에서 느슨하게 늘어뜨립니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 115200 baud로 엽니다.',
    ],
    variables: {
    ...
    analysis: [
      '두 센서 주소가 0x68과 0x69로 나뉘어 각각 읽히는지 확인합니다.',
    ...
    checkpoints: [
      ...
      { sign: '붙는 충돌인데 두 속도가 다릅니다', meaning: '붙지 않았거나 충돌 후 회전이 생긴 것입니다. 연결 상태를 확인하세요.' },
    ],
    extensions: {
  ...
  'ph05-restitution-coefficient': {
    ...
    setup: [
      '단단하고 수평한 바닥을 고르고, 그 위 천장이나 선반에 HC-SR04를 바닥을 향하도록 고정합니다.',
      '센서 바로 아래 바닥에 공이 떨어질 자리를 표시합니다.',
      '센서에서 바닥까지의 거리를 줄자로 재어 적습니다.',
      '공을 항상 같은 높이에서 놓을 수 있도록 낙하 가이드로 쓸 관이나 자를 세웁니다.',
      '시험할 공 여러 종류에 이름표를 붙여 순서를 정합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '공을 놓지 않은 상태의 거리값을 읽어 바닥까지의 기준 거리로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph06-spring-oscillation': {
    ...
    setup: [
      '스탠드를 책상에 단단히 고정하고 용수철을 걸 고리를 답니다.',
      '용수철 아래에 질량추를 달고, 추와 MPU6050을 합친 질량을 저울로 재어 적습니다.',
      'MPU6050을 질량추에 테이프로 단단히 붙이고, 센서의 한 축이 위아래 방향과 나란해지도록 맞춥니다.',
      '추 아래 바닥에 방석을 놓아 용수철이 빠졌을 때 다치지 않게 합니다.',
      '점퍼선이 추의 위아래 움직임을 방해하지 않도록 넉넉히 늘어뜨립니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '추를 가만히 매단 채 값을 읽어 정지 상태의 기준값을 적어 둡니다.',
    ],
    variables: {
  ...
  'ph07-centripetal-acceleration': {
    ...
    setup: [
      '저속 회전판을 책상에 고정하고 흔들리지 않는지 손으로 눌러 확인합니다.',
      '회전판 위에 반지름 눈금을 자로 그려 두거나 자를 붙입니다.',
      'MPU6050을 정한 반지름 자리에 테이프로 단단히 붙이고, 센서의 한 축이 회전 중심을 향하도록 돌립니다.',
      '센서 중심에서 회전 중심까지의 거리를 자로 재어 반지름으로 적습니다.',
      '회전판 위에 투명 보호 덮개를 씌워 손과 선이 말려 들어가지 않게 합니다.',
      '선이 회전판에 감기지 않도록 위쪽으로 빼서 고정합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ],
    variables: {
    ...
    analysis: [
      ...
      '회전이 안정된 구간에서 가속도와 각속도의 평균을 구합니다.',
  ...
  'ph08-rpm-comparison': {
    ...
    setup: [
      '저속 회전판을 책상에 고정하고 흔들리지 않는지 확인합니다.',
      'MPU6050을 회전판 위 중심 가까이에 붙이고, z축이 회전축과 나란해지도록 맞춥니다.',
      '작은 자석을 회전판 가장자리에 강력 테이프로 단단히 붙입니다.',
      '붙인 자석의 개수를 세어 적습니다. 한 바퀴에 몇 번 펄스가 나올지 정하는 값입니다.',
      'HBE0704를 회전판 옆 고정된 받침에 세워, 자석이 5 mm 안쪽으로 스쳐 지나가게 자리를 잡습니다.',
      '회전판 위에 보호 덮개를 씌웁니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '회전판을 손으로 천천히 한 바퀴 돌리며 자석이 지날 때 값이 크게 떨어지는지 확인합니다.',
    ],
    variables: {
  ...
  'ph09-friction-coefficients': {
    ...
    setup: [
      '각도를 바꿀 수 있는 경사면을 책상에 놓고, 각도기를 옆에 붙여 각을 읽을 수 있게 합니다.',
      '시험할 재료를 경사면 위에 놓을 나무토막의 바닥에 각각 붙여 둡니다.',
      'MPU6050을 나무토막 위에 붙이고, 센서의 x축이 미끄러지는 방향과 나란해지도록 돌립니다.',
      '경사면 아래쪽 끝에 받침을 두어 토막이 바닥으로 떨어지지 않게 합니다.',
      '점퍼선이 토막을 잡아당기지 않도록 위쪽에서 느슨하게 늘어뜨립니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '경사면을 수평으로 둔 채 값을 읽어 x축 가속도가 0 근처인지 확인하고 그 값을 영점으로 적습니다.',
    ],
    variables: {
  ...
  'ph10-rotational-damping': {
    ...
    setup: [
      '회전판을 책상에 고정하고 손으로 돌렸을 때 오래 도는지 확인합니다.',
      'MPU6050을 회전판 중심 가까이에 붙여 z축이 회전축과 나란해지게 맞춥니다.',
      '작은 자석을 회전판 가장자리에 단단히 붙이고 개수를 적습니다.',
      'HBE0704를 회전판 옆 고정 받침에 세워 자석이 스쳐 지나가게 자리를 잡습니다.',
      '제동 재료로 쓸 천이나 스펀지를 종류별로 준비해 이름표를 붙입니다.',
      '회전판 위에 보호 덮개를 씌우고 선을 위쪽으로 빼서 고정합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ],
    variables: {
  ...
  'ph11-specific-heat': {
    ...
    setup: [
      '같은 크기의 단열 용기 두 개를 준비하고 각각 1번, 2번으로 표시합니다.',
      '시료 두 가지를 같은 질량이 되도록 저울로 재어 각 용기에 담습니다.',
      'DS18B20 두 개에 번호표를 붙이고 각각 한 용기의 시료 가운데 깊이에 고정합니다.',
      '저전압 정격 히터를 두 용기에 같은 방식으로 담그고, 전력계를 히터 회로에 연결합니다.',
      '두 용기의 뚜껑을 덮고 바람이 닿지 않는 자리에 나란히 놓습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
  ...
  'ph12-latent-heat': {
    ...
    setup: [
      '잘게 부순 얼음을 단열 컵에 담고 저울로 질량을 재어 적습니다.',
      'DS18B20 프로브를 얼음 가운데에 묻히도록 꽂아 고정합니다.',
      '저전압 정격 히터를 컵에 담그고 전력계를 히터 회로에 연결합니다.',
      '컵에 뚜껑을 덮되 저을 수 있도록 막대를 넣을 틈을 남깁니다.',
      '컵을 바람이 닿지 않는 자리에 놓습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '히터를 켜기 전에 온도가 0도 근처에서 안정될 때까지 기다리며 시작 온도를 적습니다.',
    ],
    variables: {
  ...
  'ph13-thermal-conductivity': {
    ...
    setup: [
      '같은 길이와 굵기의 재료 막대를 종류별로 준비해 이름표를 붙입니다.',
      '막대 한쪽 끝에 저전압 히터를 붙이고, 두 곳 사이 거리를 자로 재어 적습니다.',
      'DS18B20 두 개를 막대의 히터 쪽 끝과 반대쪽 끝에 열전도 테이프로 붙입니다.',
      '막대 둘레를 단열재로 감싸 옆으로 새는 열을 줄입니다.',
      '막대를 바람이 닿지 않는 자리에 놓고 손으로 만지지 않습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '히터를 켜기 전에 두 온도가 같아질 때까지 기다리며 초기 온도를 적습니다.',
    ],
    recording: 'curve',
  ...
  'ph14-insulation-performance': {
    ...
    setup: [
      '같은 크기와 모양의 용기 두 개를 준비하고 각각 1번, 2번으로 표시합니다.',
      '한 용기만 시험할 단열재로 감싸고, 다른 하나는 감싸지 않은 대조군으로 둡니다.',
      '두 용기에 같은 온도, 같은 양의 따뜻한 물을 담습니다.',
      'DS18B20을 단열재로 감싼 용기의 물 가운데 깊이에 고정합니다.',
      'BME280을 두 용기 사이 같은 높이에 두어 주변 공기의 온도와 습도를 함께 기록합니다.',
    ...
    analysis: [
      'BME280으로 주변 온도를 기록해 일정했는지 확인합니다.',
  ...
  'ph15-gas-temperature-pressure': {
    ...
    setup: [
      '압력을 뺄 수 있는 교육용 밀폐 용기를 준비하고 뚜껑의 잠금 상태를 확인합니다.',
      'BME280을 용기 안쪽에 고정하고, 선이 지나갈 구멍을 실리콘으로 메워 공기가 새지 않게 합니다.',
      '온수 욕조와 냉수 욕조를 각각 준비하고 물 온도를 온도계로 재어 적습니다.',
      '용기를 욕조에 담갔을 때 완전히 잠기되 넘치지 않을 물 높이를 맞춥니다.',
      '용기 안 공기의 부피가 실험 내내 변하지 않는지 확인합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '실온에서 용기 안의 온도와 압력을 읽어 기준값으로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph16-altitude-pressure': {
    ...
    setup: [
      '측정할 층을 정하고 각 층의 바닥 높이를 줄자나 건물 도면으로 재어 적습니다.',
      'BME280을 손의 열이 닿지 않도록 작은 상자에 넣어 들고 다닐 수 있게 만듭니다.',
      '노트북과 아두이노를 함께 들고 다닐 수 있도록 선을 정리합니다.',
      '엘리베이터는 문이 열고 닫힐 때 압력이 크게 흔들리므로 계단으로 이동할 길을 정합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '첫 층에서 값이 안정될 때까지 1분쯤 기다린 뒤 기준 기압을 적습니다.',
    ],
    variables: {
    ...
    analysis: [
      ...
      '기준층에서 다시 재어, 날씨 때문에 기준값이 흘러가지 않았는지 확인합니다.',
  ...
  'ph17-ohms-law': {
    ...
    setup: [
      '브레드보드에 INA219를 꽂고, 측정할 저항을 갈아 끼울 자리를 한 줄 비워 둡니다.',
      '1 킬로옴, 2.2 킬로옴, 4.7 킬로옴 저항을 색띠로 구별해 순서대로 늘어놓습니다.',
      '100 옴 필터 저항과 470 마이크로패럿 커패시터를 전원 쪽에 꽂아 전압이 출렁이지 않게 합니다.',
      '전해 커패시터의 긴 다리가 플러스 쪽인지 확인합니다. 거꾸로 꽂으면 터질 수 있습니다.',
      '화면 위쪽 **2. 배선하기**를 끝까지 마친 뒤, 전류가 INA219의 VIN+에서 저항을 지나 VIN-로 흐르는지 손가락으로 짚어 확인합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ...
    analysis: [
      ...
      '4.7 kΩ 조건은 전류가 0.1~1 mA로 작아 눈금 몇 칸에 불과하므로, 같은 조건을 여러 번 재어 평균과 불확도를 적습니다.',
  ...
  'ph18-series-parallel-resistance': {
    ...
    setup: [
      '브레드보드에 INA219를 꽂고 220 옴 저항과 1 킬로옴 저항을 색띠로 구별해 준비합니다.',
      '먼저 두 저항을 한 줄로 이어 직렬 회로를 만듭니다.',
      '5V 저전압 전원을 준비합니다. 3V에서는 직렬 전류가 너무 작아 센서가 구별하지 못합니다.',
      '화면 위쪽 **2. 배선하기**를 끝까지 마친 뒤, INA219가 전원과 저항망 사이에 끼어 있는지 확인합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '직렬 조건의 값을 읽어 코드의 조건 이름을 직렬로 맞추고, 나중에 병렬로 바꿀 때 점퍼를 어디로 옮길지 미리 정해 둡니다.',
    ],
    variables: {
  ...
  'ph19-kirchhoff-laws': {
    ...
    setup: [
      '브레드보드에 220 옴, 470 옴, 1 킬로옴 저항을 그림처럼 두 갈래 병렬 회로로 꽂습니다.',
      '각 저항에 이름표를 붙이고 어느 갈래가 몇 번인지 노트에 회로도를 그려 적습니다.',
      '측정점을 옮길 때 쓸 점퍼선을 미리 잘라 준비합니다.',
      'INA219를 첫 번째 측정점에 끼워 넣습니다.',
      '화면 위쪽 **2. 배선하기**를 끝까지 마친 뒤 전류가 갈라지는 지점과 다시 만나는 지점을 손가락으로 짚어 확인합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ...
    checkpoints: [
      ...
      { sign: '두 가지의 전압이 다릅니다', meaning: '병렬이 아닌 배선입니다. 두 저항이 같은 두 마디를 공유하는지 확인하세요.' },
    ],
    extensions: {
  ...
  'ph20-joule-heating': {
    ...
    setup: [
      '10 옴 전력저항을 단열 용기 안에 넣고 열전도 테이프로 DS18B20 프로브를 저항 몸통에 밀착시켜 붙입니다.',
      '저항과 프로브가 서로 떨어지지 않는지 살짝 당겨 확인합니다. 떨어지면 열이 전해지지 않습니다.',
      '단열 용기에 뚜껑을 덮고 바람이 닿지 않는 자리에 놓습니다.',
      '3에서 5V 사이의 전류 제한 전원을 준비하고 전류 제한을 저항 정격 안으로 맞춥니다.',
      '브레드보드에 INA219를 꽂고 전류가 저항을 지나가도록 직렬로 넣습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '전원을 켜기 전에 온도가 실온에서 안정될 때까지 기다리며 초기 온도를 적습니다.',
    ],
    variables: {
  ...
  'ph21-rc-time-constant': {
    ...
    setup: [
      '브레드보드에 10 킬로옴 저항과 100 마이크로패럿 전해 커패시터를 직렬로 꽂습니다.',
      '전해 커패시터의 긴 다리가 플러스 쪽인지 확인합니다. 거꾸로 꽂으면 터질 수 있습니다.',
      '커패시터의 플러스 극을 A0에도 연결해 전압을 함께 읽을 수 있게 합니다.',
      '기록을 시작하기 전에 10 킬로옴 저항을 커패시터 양 끝에 잠시 대어 완전히 방전시킵니다.',
      '충전에서 방전으로 바꿀 때 점퍼를 어디로 옮길지 미리 정하고 회로도에 표시해 둡니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '시리얼 모니터를 먼저 열어 둔 뒤에 전원을 연결합니다. 그래야 올라가기 시작하는 첫 순간이 기록에 담깁니다.',
    ],
    variables: {
  ...
  'ph22-battery-internal-resistance': {
    ...
    setup: [
      '새 9V 사각 건전지를 홀더에 끼웁니다. 1.5V 건전지는 전압 강하가 너무 작아 이 센서로는 잴 수 없습니다.',
      '100 옴, 220 옴, 470 옴 정격저항을 1와트 이상으로 준비해 색띠로 구별해 둡니다.',
      '브레드보드에 INA219를 꽂고 저항을 갈아 끼울 자리를 한 줄 비워 둡니다.',
      '화면 위쪽 **2. 배선하기**를 끝까지 마친 뒤, 전류가 건전지에서 INA219를 지나 저항으로 흐르는지 확인합니다.',
      '저항이 뜨거워질 수 있으므로 손으로 만지지 않고 집게로 갈아 끼울 준비를 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '저항을 꽂지 않은 상태의 전압을 읽어 부하가 없을 때의 전압으로 적어 둡니다.',
    ],
    variables: {
    ...
    analysis: [
      '저항을 떼고 개방 상태(OPEN)의 전압을 기록합니다. 이 값이 기전력에 가깝습니다.',
  ...
  'ph23-solar-iv-mpp': {
    ...
    setup: [
      '소형 태양전지를 받침대에 고정하고 빛을 받는 면이 광원을 정면으로 보게 맞춥니다.',
      '일정한 밝기의 스탠드를 태양전지 앞 정해진 거리에 세우고 자리를 바닥에 표시합니다.',
      '100 옴부터 10 킬로옴까지의 저항을 큰 값에서 작은 값 순서로 늘어놓고 색띠를 확인합니다.',
      'TSL2591을 태양전지 옆 같은 높이에 두어 같은 빛을 받게 고정합니다.',
      '브레드보드에 INA219를 꽂고 태양전지와 저항 사이에 직렬로 넣습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '가장 큰 저항부터 꽂고 값이 안정될 때까지 10초를 기다립니다.',
    ],
    variables: {
  ...
  'ph24-solenoid-current-field': {
    ...
    setup: [
      '교육용 솔레노이드를 나무나 플라스틱 받침에 고정합니다. 쇠받침은 자기장을 흐트러뜨립니다.',
      'HBE0704를 비자성 지그에 붙여 솔레노이드 축의 가운데에 오도록 자리를 잡습니다.',
      '센서에서 솔레노이드 끝까지의 거리를 자로 재어 적고, 실험 내내 옮기지 않습니다.',
      '솔레노이드 규격에 맞는 직렬 전력저항을 골라 브레드보드에 꽂습니다.',
      '전류 제한 전원의 전류 한계를 솔레노이드 정격 안으로 미리 맞춥니다.',
      '브레드보드에 INA219를 꽂아 솔레노이드에 흐르는 전류가 센서를 지나가게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '전원을 끈 상태에서 홀 센서 값을 100번 읽어 평균을 내고 영점으로 적어 둡니다.',
    ],
    variables: {
    ...
    analysis: [
      '전류를 0으로 한 I000 조건에서 홀 센서의 영점을 구합니다.',
    ...
    checkpoints: [
      ...
      { sign: '전류가 0인데 홀 값이 0이 아닙니다', meaning: '정상입니다. 그것이 영점이며, 빼고 분석해야 합니다.' },
    ],
    extensions: {
  ...
  'ph25-coil-turns-field': {
    ...
    setup: [
      '길이와 지름이 같고 감은 수만 다른 코일 세 개를 준비해 감은 수를 각각 적어 붙입니다.',
      '코일을 끼울 나무나 플라스틱 받침을 만들어 세 코일을 같은 자리에 놓을 수 있게 합니다.',
      'HBE0704를 비자성 지그에 붙여 코일 축의 같은 자리에 오도록 고정합니다.',
      '센서에서 코일 끝까지의 거리를 자로 재어 적고, 코일을 바꿀 때마다 같은 거리인지 확인합니다.',
      '코일 규격에 맞는 직렬 전력저항을 꽂고 전류 제한 전원의 한계를 맞춥니다.',
      '브레드보드에 INA219를 꽂아 코일 전류가 센서를 지나가게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '전원을 끈 상태에서 홀 센서의 영점을 재어 적어 둡니다.',
    ],
    variables: {
    ...
    analysis: [
      '코일을 바꿀 때마다 current_mA를 확인해 세 조건의 전류가 같은지 검증합니다.',
  ...
  'ph26-rotating-magnet-signal': {
    ...
    setup: [
      '손으로 돌리는 원판이나 저속 모터를 책상에 고정합니다.',
      '자석을 원판 가장자리에 강력 테이프로 단단히 붙이고, 붙인 개수를 적습니다.',
      'HBE0704를 원판 옆 고정 받침에 세워 자석이 5 mm 안쪽으로 스쳐 지나가게 자리를 잡습니다.',
      '원판 위에 투명 보호 덮개를 씌워 손이 들어가지 않게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '원판을 아주 천천히 한 바퀴 돌리며 자석이 지날 때 값이 크게 변하는지 확인합니다.',
    ],
    variables: {
  ...
  'ph27-magnetic-shielding': {
    ...
    setup: [
      '자석을 나무나 플라스틱 받침에 고정해 실험 내내 움직이지 않게 합니다.',
      'HBE0704를 자석에서 일정 거리 떨어진 비자성 지그에 고정하고, 그 거리를 자로 재어 적습니다.',
      '철, 알루미늄, 플라스틱 시료를 두께별로 준비해 이름표를 붙입니다.',
      '자석과 센서 사이에 시료를 끼워 넣을 홈을 지그에 만들어, 시료를 바꿔도 거리가 변하지 않게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '시료를 아무것도 끼우지 않은 상태의 값을 읽어 기준값으로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph28-malus-law': {
    ...
    setup: [
      '차광통 한쪽 끝에 일정한 밝기의 LED 광원을 고정합니다.',
      '차광통 반대쪽 끝에 TSL2591을 수광창이 광원을 정면으로 보도록 넣어 고정합니다.',
      '두 장의 편광판을 광원과 센서 사이에 차례로 세웁니다.',
      '앞쪽 편광판은 움직이지 않게 고정하고, 뒤쪽 편광판만 돌릴 수 있도록 각도 눈금판에 붙입니다.',
      '눈금판의 0도가 두 편광판의 방향이 같아지는 자리가 되도록 맞춥니다.',
      '차광통의 틈을 검은 천으로 막아 바깥 빛이 들어오지 않게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '광원을 끈 채 값을 읽어 새어 드는 빛을 확인하고, 0에 가깝지 않으면 틈을 더 막습니다.',
    ],
    variables: {
  ...
  'ph29-transmittance-absorbance': {
    ...
    setup: [
      '차광통 한쪽 끝에 일정한 광원을, 반대쪽 끝에 TSL2591을 마주 보도록 고정합니다.',
      '광원과 센서 사이에 큐벳을 세울 자리를 정하고 홈을 만들어 항상 같은 자리에 놓이게 합니다.',
      '같은 모양의 큐벳을 시료 수만큼 준비해 이름표를 붙입니다.',
      '가장 옅은 농도부터 진한 농도까지 색소 용액을 만들어 큐벳에 채웁니다.',
      '차광통의 틈을 검은 천으로 막습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '아무것도 넣지 않은 상태와 물만 넣은 큐벳의 값을 각각 읽어 기준값으로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph30-reflection-intensity-angle': {
    ...
    setup: [
      '회전 눈금판 가운데에 거울이나 시료를 놓을 자리를 정하고 표시합니다.',
      '좁은 LED 광원을 눈금판 가장자리에 고정해 시료 가운데를 비추게 맞춥니다.',
      'TSL2591을 눈금판 위에서 각도를 바꿔 가며 옮길 수 있는 팔에 붙입니다.',
      '센서와 시료 사이 거리를 자로 재어 적고, 각도를 바꿔도 그 거리가 변하지 않게 합니다.',
      '광원에서 센서로 빛이 곧바로 들어가지 않도록 사이에 차광판을 세웁니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '광원을 끈 채 값을 읽어 주변광으로 적어 둡니다.',
    ],
    variables: {
  ...
  'ph31-lens-focal-length': {
    ...
    setup: [
      '책상 위에 광학 레일이나 긴 자를 놓고 눈금이 잘 보이게 붙입니다.',
      '레일 한쪽 끝에 LED 물체를 세우고 자리를 표시합니다.',
      '볼록렌즈를 레일 위에서 앞뒤로 옮길 수 있는 받침에 세웁니다.',
      'TSL2591을 레일 반대쪽 끝의 받침에 붙여 수광창이 렌즈를 정면으로 보게 합니다.',
      '센서 앞에 작은 구멍을 뚫은 차광판을 세워 초점이 맞을 때만 빛이 많이 들어오게 합니다.',
      '물체, 렌즈, 센서의 높이를 모두 같게 맞춥니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
    ],
    variables: {
  ...
  'ph32-aperture-light': {
    ...
    setup: [
      '차광통 한쪽 끝에 일정한 광원을, 반대쪽 끝에 TSL2591을 마주 보도록 고정합니다.',
      '구멍 지름이 다른 차광판을 여러 장 준비합니다.',
      '각 차광판의 구멍 지름을 버니어캘리퍼스로 재어 판마다 적어 붙입니다.',
      '차광판을 끼울 홈을 광원과 센서 사이에 만들어 판을 바꿔도 자리가 변하지 않게 합니다.',
      '차광통의 틈을 검은 천으로 막습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '광원을 끈 채 값을 읽어 새어 드는 빛을 확인합니다.',
    ],
    variables: {
  ...
  'ph33-light-source-stability': {
    ...
    setup: [
      '비교할 광원들을 정하고 각각 이름표를 붙입니다.',
      '차광통 안에 TSL2591을 고정하고, 광원을 끼울 자리를 정해 표시합니다.',
      '광원을 바꿔도 센서와의 거리와 각도가 변하지 않도록 지그를 만듭니다.',
      '차광통의 틈을 검은 천으로 막아 바깥 빛이 섞이지 않게 합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '첫 광원을 켜고 10분쯤 두어 밝기가 안정된 뒤부터 기록을 시작합니다.',
    ],
    variables: {
  ...
  'ph34-torricelli-drain': {
    ...
    setup: [
      '안지름 12 cm 이상인 넓은 투명 원통 용기를 준비합니다. 좁은 용기는 초음파가 벽에 먼저 부딪힙니다.',
      '용기 옆면 아래쪽에 작은 배출구를 내고 마개를 끼웁니다.',
      '용기 아래에 물받이를 놓아 흘러나온 물을 받게 합니다.',
      'HC-SR04를 용기 위에 수면을 향하도록 고정하고, 센서와 수면 사이에 방수 차폐판을 둡니다.',
      '센서에서 용기 바닥까지의 거리를 줄자로 재어 적습니다.',
      '배출구의 지름을 자로 재어 적습니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '마개를 막은 채 물을 채우고 수면이 잔잔해질 때까지 기다린 뒤 시작 수위를 읽습니다.',
    ],
    variables: {
  ...
  'ph35-temperature-speed-of-sound': {
    ...
    setup: [
      '책상 위에 줄자를 펴서 붙이고 한쪽 끝에 HC-SR04를 정면을 보도록 고정합니다.',
      '평평하고 단단한 반사판을 줄자 위 정해진 거리에 센서와 마주 보도록 세웁니다.',
      '센서 앞면에서 반사판까지의 거리를 줄자로 재어 기준 거리로 적습니다.',
      'BME280을 센서와 반사판 사이 공기 중에 두어 소리가 지나는 길의 온도를 재게 합니다.',
      '온도를 바꿀 방법을 정합니다. 창문을 열거나 다른 방으로 옮기는 정도면 충분합니다.',
      'USB 케이블을 연결하고 시리얼 모니터를 9600 baud로 엽니다.',
      '현재 온도에서 거리값을 읽어 줄자로 잰 실제 거리와 얼마나 차이 나는지 적어 둡니다.',
    ],
    variables: {
