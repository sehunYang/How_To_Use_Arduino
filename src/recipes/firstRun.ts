...
export function firstRunTroubleshooting(baudRate: number): TroubleshootingItem[] {
  return [
    ...
    {
      symptom: '데이터 화면에서 [USB로 받기]를 눌러도 포트가 열리지 않음',
      cause: '아두이노 IDE의 시리얼 모니터가 포트를 붙잡고 있습니다. 포트는 한 번에 한 프로그램만 쓸 수 있습니다.',
      fix: '시리얼 모니터 창을 닫고 다시 [USB로 받기]를 누르세요. 거꾸로 데이터 화면이 받는 중이면 IDE 업로드가 안 되니, 코드를 다시 올리기 전에 [멈추고 회차로 넣기]나 [버리기]를 누르세요.',
    },
    // 여기까지는 코드를 그대로 쓴 학생이 겪는 일입니다. 아래 둘은 코드를 고쳐 본
    // 학생만 만납니다. 값을 바꿔 보라고 권해 놓고 그때 나오는 오류를 아무 데도
    // 적어 두지 않으면, 학생은 자기가 무언가를 망가뜨렸다고 여기고 되돌립니다.
    {
      symptom: '값을 고친 뒤 빨간 글씨로 expected ‘;’ before … 가 뜸',
      cause: '고친 줄에서 마침표 노릇을 하는 세미콜론(;)이나 따옴표·괄호의 짝이 빠졌습니다.',
      fix: '빨간 글씨가 가리키는 줄과 그 바로 윗줄을 보세요. 줄 끝에 `;`가 있는지, 여는 괄호마다 닫는 괄호가 있는지 확인합니다. 찾기 어려우면 [코드 복사]로 원래 코드를 다시 붙여 넣고 한 곳만 다시 고치세요.',
    },
    {
      symptom: '값을 고친 뒤 was not declared in this scope가 뜸',
      cause: '이름을 잘못 적었거나, 그 이름을 만든 줄까지 함께 지웠습니다. 아두이노는 대문자와 소문자를 다른 이름으로 봅니다.',
      fix: '빨간 글씨에 적힌 이름을 코드에서 찾아 철자와 대소문자가 같은지 확인하세요. 그 이름을 만드는 줄이 사라졌다면 [코드 복사]로 원래 코드를 되돌린 뒤 다시 고치세요.',
    },
  ]
}
