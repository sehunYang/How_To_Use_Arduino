...
/**
 * 브레드보드에서 어떤 구멍이 서로 이어져 있는지.
 *
 * 배선 단계는 "어디에 꽂는가"만 알려 주므로, 한 칸 밀려 꽂아도 화면은 그대로
 * 다음 단계로 넘어갑니다. 그때 학생이 스스로 되짚으려면 **왜 그 구멍인지**를
 * 알아야 하는데, 지금까지 브레드보드 설명은 준비물의 한 줄이 전부였습니다.
 * 레시피마다 다르지 않은 내용이라 여기 한곳에 둡니다.
 */
export const breadboardBasics: FirstRunStep[] = [
  {
    title: '세로 다섯 구멍이 한 묶음입니다',
    detail: '같은 묶음에 꽂은 다리끼리는 선으로 이은 것과 똑같습니다. 부품 다리와 점퍼선을 같은 묶음에 꽂아 이어 줍니다.',
  },
  {
    title: '가운데 홈에서 위아래가 끊깁니다',
    detail: '홈 위쪽 묶음과 아래쪽 묶음은 서로 이어져 있지 않습니다. 다리가 여러 개인 부품을 홈에 걸쳐 꽂는 이유입니다.',
  },
  {
    title: '가장자리 빨강(+)·파랑(−) 줄은 가로로 깁니다',
    detail: '이 두 줄만 세로가 아니라 가로로 이어져 있어 5V와 GND를 여러 부품에 나눠 줍니다.',
  },
  {
    title: '한 부품의 두 다리를 같은 묶음에 꽂으면 안 됩니다',
    detail: '양 끝이 곧바로 붙어 버려 부품이 아무 일도 하지 않습니다. 저항이 빠진 것과 같은 상태가 됩니다.',
  },
  {
    title: '값이 이상하면 한 칸 밀렸는지부터 보세요',
    detail: '한 칸 옆에 꽂으면 다른 묶음이 되어 연결이 끊깁니다. 초보자가 가장 자주 겪는 일이고, 화면으로는 알 수 없습니다.',
  },
]
